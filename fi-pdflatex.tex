...
%%               %% document
  ...
  nocolorbold, %% The `nocolorbold` option disables the usage of the
               %% blue color for bold text, instead using black. Replace with `colorbold` to use blue for b old text.
...
\thesissetup{
    ...
    abstract    = {%
        This thesis explores the potential of generative AI text models to simplify time tracking for employees working in the field. The goal is to develop a cloud-based application that allows users to log their working hours by recording audio descriptions of their tasks, also specifying the client and project. This recorded speech is then transcribed into text and converted into structured database entries. The research evaluates the reliability and practicality of this approach in real-world conditions.
        
        In addition to automated transcription, the application provides features for replaying audio recordings, manually editing and managing entries as well as basic reports functionality. By simplifying the logging process, this solution aims to minimize forgetfulness and improve accuracy of work records while offering employees an efficient way to track their work.
    },
    thanks      = {%
...
\begin{itemize}
    ...
    \begin{compactitem}
        ...
        \item \textbf{Text-to-Timesheet Endpoint} --- Converts the transcribed text into a structured format by generating Timesheet attributes.
    \end{compactitem}
    \item \textbf{Firebase Services}
    \begin{compactitem}
        \item \textbf{Firebase Storage} --- Used for storing audio files.
        \item \textbf{Firestore Database} --- A \acrshort{NoSQL} database used to store static content such as activities, clients, and projects, as well as dynamic data such as Timesheets.
